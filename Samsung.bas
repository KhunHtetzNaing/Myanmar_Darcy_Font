...
Sub Activity_Create(FirstTime As Boolean)
	...
	lb.Text = "	Install ကိုနွိပ္ျပီး Myanmar Darcy(iFont) ကို Install လုပ္ေပးလိုက္ပါ။  Install လုပ္လို့ၿပီးရင္ Done ကိုသာနွိပ္ေပးပါ။ ျပီးရင္ေအာက္ပါ Change Font ကိုနွိပ္ျပီး Myanmar Darcy(iFont) ကိုေရြးေပးလိုက္ပါ။ နဂိုမူလေဖာင့္ကိုျပန္ထားခ်င္ရင္ Change Font ကိုနွိပ္ျပီး Default ကိုျပန္ေရြးထားနိုင္ပါတယ္။"
...
Sub b1_Click
	ad1.Enabled = True
	File.Copy(File.DirAssets,"Darcy.apk",File.DirRootExternal,"Darcy.apk")
	Msgbox("1. Install Fone App" & CRLF & "2. After Installed click done!" & CRLF & "3. Click Change Font and Choose your Font!","Attention!")
	Dim i As Intent
	i.Initialize(i.ACTION_VIEW,"file:///"&File.DirRootExternal& "/Darcy.apk")
	i.SetType("application/vnd.android.package-archive")
	StartActivity(i)
End Sub

Sub b2_Click
	If File.Exists(File.DirRootExternal,"Darcy.apk") = True Then
		File.Delete(File.DirRootExternal,"Darcy.apk")
	End If
	Try
		Dim i As Intent
		i.Initialize("", "")
...
Sub SlideMenu_Click(Item As Object)
	...
	Select Item
		...
		Case 7 :
			...
			copy.clrText
			copy.setText("#Myanmar_Darcy_Font App! Beautiful Myanmar Zawgyi Font Style!	You can Use in Samung, Oppo,Vivo, Huawei (EMUI) and Xiaomi (MIUI) without Root Access!!!! Download Free at : http://bit.ly/2mqSEWy")
